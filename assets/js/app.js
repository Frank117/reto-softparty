(function () {
    ...
    function onFacturarClick(ev) {
        ...
        const duracionMs = Math.max(0, salida - entrada);
        const horas = Math.ceil(duracionMs / HORA_EN_MS) || 1;
        const total = horas * TARIFA_POR_HORA;
        generarFacturaPDF({ ...registro }, entrada, salida, horas, total);
    ...
    function generarFacturaPDF(reg, entrada, salida, horas, total) {
        ...
        doc.text(`Tarifa por hora: ${formatCurrency(TARIFA_POR_HORA)}`, 14, 32);
        const datos = [ ['Nombre', reg.nombre], ['Documento', reg.documento], ['Contacto', reg.contacto], ['Placa', reg.placa], ['Tipo', reg.tipo], ['Entrada', formatDateTime(entrada)], ['Salida', formatDateTime(salida)], ['Tiempo facturado', `${horas} hora(s)`], ['Valor a pagar', formatCurrency(total)] ];
